...
同时提供了消息推送功能，支持把设备上报的实时数据推送到第三方服务器，用于二次数据分析和开发。


### 开发流程

仅设备接入的云对接方式，可按照以下流程进行开发。
...
登录[AIOT开放平台](https://opencloud.aqara.cn/)，在右上角点击进入“控制台”。选择“应用管理”-“新建应用”，“应用类型”选择“设备接入”，上传应用图标、输入应用名称、行业类型、简介等信息，完成创建。应用审核后，系统会自动分配**Appid**和**Appkey**。
...
登录[AIOT开放平台](https://opencloud.aqara.cn/)，进入控制台，展开APi参考菜单栏，选择**设备接入API**，即可查看接口介绍和进行接口调试。
