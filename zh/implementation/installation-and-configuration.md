...
| 设备   | 不可操控传感类设备，仅网关和控制类设备                  | 可以调用所有设备进行操作                             |
| 显示   | 添加场景后，在首页控制项中默认显示快捷图标，在场景页面显示所有场景的列表 | 添加自动化后，在自动化页面显示列表，如有需要，可手动设置在首页控制项显示快捷图标 |
|      |                                      |                                          |
...
2. 单击右上角的”+“，用户自行选择使用系统默认的场景或添加自定义场景：

   - 系统默认场景（起床、回家、离家、睡觉）

     选择任一默认场景，进入”编辑场景“页面，系统默认已添加相关动作，您可自行删除不需要的动作，同时添加需要的动作。
