...
| 更新时间       | 文档版本   | 更新日志                                     |
| ---------- | ------ | ---------------------------------------- |
| 2017-10-09 | V2.0.1 | 修改：天然气报警器、烟雾报警器和水浸传感器的属性上报               |
...
使用局域网通信对接前，需先开启局域网通信协议功能。由于局域网通信采用key加密方式，因此还需在米家智能家庭APP上获取随机生成的网关KEY（使用AES-CBC 128加密，16个字节长度的字符串）。开启局域网通信协议并拥有该网关的KEY后，才能与该网关进行局域网通信。 
...
> 注意：若“关于”页面没有显示“局域网通信协议”，iOS系统在此页面空白处连续点击10次即可出现；安卓系统连续点击“插件版本”即可出现。
...
> 注意：“token”为网关生成的随机字符串，每10s刷新一次，用户可用此token来生成写设备时的“key”。
>
...
```
{
   ...
   "model":"gateway.v3",
   "sid":"f0b429b3c9d9",
   "params":[{"rgb":4278255360},{"illumination":350},{"proto_version":"2.0.0"}
...
> 注意：“key”为32个字节长度的字符串。当网关启用了加密模式时，会对该key进行解密并校验，以验证写命令的合法性。该“key”的生成规则是：用户收到心跳“heartbeat”里的16个字节的“token”字符串之后，使用网关的KEY（在米家智能家庭APP里获取的随机KEY）对该字符串进行AES-CBC 128加密，生成16个字节的密文后，再转换为32个字节的ASCII码字符串。
>
